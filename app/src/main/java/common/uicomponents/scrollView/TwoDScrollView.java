package common.uicomponents.scrollView;
...
public class TwoDScrollView extends FrameLayout {
    ...
    private void createScaleGestureDetector(final View childLayout) {
        mScaleDetector = new ScaleGestureDetector(getContext(), new ScaleGestureDetector.SimpleOnScaleGestureListener() {
            ...
            public boolean onScale(final ScaleGestureDetector detector) {
                ...
                mScale = Math.max(minScale, Math.min(mScale, maxScale));
                zoomChild(prevScale, mScale);
                return true;
            }
        });
    }

    private void zoomChild(float prevScale, float mScale) {
        if (scaleListener != null) {
            scaleListener.onScaleChange(prevScale, mScale);
        }
    }
    ...
    private boolean canScroll() {
        ...
        if (child != null) {
            ...
            boolean canScroll = (getHeight() < childHeight + getPaddingTop() + getPaddingBottom()) ||
    ...
    protected int computeVerticalScrollRange() {
        ...
        return (int) (scrollRange * mScale + getScrollVerticalMin());
    }
    ...
    protected int computeHorizontalScrollRange() {
        ...
        return (int) (scrollRange * mScale + getScrollHorizontalMin());
    }
    ...
    private int getScrollVerticalMin() {
        ...
        return (int) ((1 - mScale) * child.getPivotY());
    }
    ...
    private int getScrollHorizontalMin() {
        if (this.getChildCount() < 1)
            return 0;
        View child = this.getChildAt(0);
        return (int) ((1 - mScale) * child.getPivotX());
    }
    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onDoubleTap(MotionEvent e) {
            float prevScale = mScale;
            float midScale = (maxScale + minScale) * 0.4f;
            if (Math.abs(prevScale - minScale) / minScale < 0.2f) {
                mScale = midScale;
            } else if (Math.abs(prevScale - midScale) / midScale < 0.2f) {
                mScale = maxScale;
            } else {
                mScale = minScale;
            }
    ...
    public interface ScaleListener {
        public void onScaleChange(float previouScale, float newScale);
    }
}